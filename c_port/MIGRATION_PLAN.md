...
- Left ASM only at strict CPU/boot boundaries:
  ...
  - dedicated ISR wrapper ASM files (`kernel/isr_timer.asm`, `kernel/isr_keyboard.asm`, `kernel/isr_common_wrapper.asm`) for prologue/epilogue + IRET.
...
- `kernel/interrupts.lasys` -> `kernel/interrupts.c` + ASM ISR wrappers (`kernel/isr_*.asm`)
